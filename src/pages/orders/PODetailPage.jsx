...
import { getInvoiceByPO, emailInvoice, openInvoicePdf } from '../../services/invoice.service'
...
export default function PODetailPage() {
  ...
  useEffect(() => {
    getPOById(id)
      .then((data) => {
        setPO(data)
        return getInvoiceByPO(id).catch(() => null)
      })
      .then(setInvoice)
  ...
  const handleEmailInvoice = async () => {
    if (!invoice?.id) {
      setEmailMsg('No invoice linked to this PO yet.')
      return
    }
    setEmailing(true)
    setEmailMsg('')
    try {
      ...
      setEmailMsg(err.response?.data?.message || err.message || 'Failed to send invoice')
    } finally {
      setEmailing(false)
    }
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {invoice?.id && (
            <Button variant="secondary" size="sm" onClick={() => openInvoicePdf(invoice.id)}>
              View PDF
            </Button>
          )}
          ...
          <Button variant="secondary" size="sm" loading={emailing} onClick={handleEmailInvoice} disabled={!invoice?.id}>
            <Mail size={13} /> Email Invoice
          </Button>
          {invoice?.id && (
            <Link to={`/invoices/${invoice.id}`} className="text-xs text-primary hover:text-blue-700">
              View Invoice →
            </Link>
          )}
