...
export default function CreateRFQPage() {
  ...
  useEffect(() => {
    getVendors()
      ...
      .catch((err) => {
        setVendors([])
        setVendorError(err.response?.data?.message || 'Failed to load vendors')
      })
  ...
  const assignableVendors = vendors.filter((v) => v.status === 'active' || v.status === 'pending')
  const filteredVendors = assignableVendors.filter(
    (v) =>
      vendorSearch === '' ||
      ...
      <div className="bg-surface border border-border rounded-lg p-6 max-w-2xl">
        ...
        {step === 2 && (
          <div className="flex flex-col gap-4">
            ...
            />
            {vendorError && (
              <p className="text-xs text-danger bg-red-50 border border-red-100 rounded px-3 py-2">{vendorError}</p>
            )}
            <div className="flex flex-col gap-1 max-h-48 overflow-y-auto border border-border rounded-lg">
              {vendorsLoading ? (
                <p className="px-3 py-4 text-sm text-text-muted">Loading vendors...</p>
              ) : filteredVendors.length === 0 ? (
                <div className="px-3 py-6 text-center flex flex-col gap-3">
                  <p className="text-sm text-text-muted">
                    {assignableVendors.length === 0
                      ? 'No vendors in the directory yet. Add a vendor or approve a vendor registration first.'
                      : 'No vendors match your search.'}
                  </p>
                  {assignableVendors.length === 0 && (
                    <Link to="/vendors/new">
                      <Button type="button" variant="secondary" size="sm">+ Register New Vendor</Button>
                    </Link>
                  )}
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-sm text-text-primary font-medium">{v.contactPerson || v.name}</p>
                        <p className="text-xs text-text-muted truncate">
                          {v.name !== v.contactPerson ? `${v.name} · ` : ''}{v.email}
                        </p>
                        <p className="text-xs text-text-muted">
                          {[v.phone && v.phone !== '—' ? v.phone : null, v.country || v.state].filter(Boolean).join(' · ')}
                        </p>
                      </div>
                    </label>
                  )
