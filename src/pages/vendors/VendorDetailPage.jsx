  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-surface border border-border rounded-lg p-5 flex flex-col gap-4">
          <h3 className="text-xs uppercase tracking-wide text-text-muted font-medium">Registration Details</h3>
          <div className="flex flex-col gap-3">
            <Row label="Full Name" value={vendor.contactPerson} />
            <Row label="Email" value={<a href={`mailto:${vendor.email}`} className="text-primary hover:text-blue-700">{vendor.email}</a>} />
            <Row label="Phone" value={vendor.phone && vendor.phone !== '—' ? vendor.phone : '—'} />
            <Row label="Country" value={vendor.country || vendor.state || '—'} />
            {vendor.additionalInfo && (
              <Row label="Additional Info" value={vendor.additionalInfo} />
            )}
          </div>
        </div>
