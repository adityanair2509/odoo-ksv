...
function AdminDashboard({ vendors, loading, categoryData, spendSummary, activities }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-3 bg-surface border border-border rounded-lg p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={categoryData.length ? categoryData : [{ category: 'No data', amount: 0 }]} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 border-t border-border pt-4">
            <div className="bg-background rounded-lg px-4 py-3">
              ...
              <p className="text-base font-bold text-text-primary mt-0.5">{formatINR(spendSummary?.ytdTotal || 0)}</p>
            </div>
            <div className="bg-background rounded-lg px-4 py-3">
              <p className="text-xs text-text-muted">Avg. Spend / Vendor</p>
              <p className="text-base font-bold text-text-primary mt-0.5">{formatINR(spendSummary?.avgPerVendor || 0)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-lg p-5">
        ...
        <Timeline entries={(activities || []).slice(0, 4)} />
      </div>
    </div>
  )
}
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════ */
function ProcurementOfficerDashboard({ pos, rfqs, loading, monthlySpend, pipeline, spendInsight, userName }) {
  const navigate = useNavigate()
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-2 bg-surface border border-border rounded-lg p-5">
          ...
          <div className="flex flex-col gap-1">
            {[
              { stage: 'Draft RFQs', count: pipeline?.draftRfqs ?? draftRFQs.length, action: () => navigate('/rfqs'), color: 'border-l-warning' },
              { stage: 'Sent RFQs', count: pipeline?.sentRfqs ?? activeRFQs.length, action: () => navigate('/rfqs'), color: 'border-l-primary' },
              { stage: 'Quotes Received', count: pipeline?.quotesReceived ?? rfqs.filter(r => r.quotationsReceived > 0).length, action: () => navigate('/quotations'), color: 'border-l-success' },
              { stage: 'Pending PO Approval', count: pipeline?.pendingPoApproval ?? 0, action: () => navigate('/purchase-orders'), color: 'border-l-warning' },
              { stage: 'PO Delivered', count: pipeline?.poDelivered ?? pos.filter(p => p.status === 'delivered').length, action: () => navigate('/purchase-orders'), color: 'border-l-success' },
        ...
        <div className="col-span-3 bg-surface border border-border rounded-lg p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={monthlySpend?.length ? monthlySpend : [{ month: '—', amount: 0 }]} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="" stroke="#F4F4F5" horizontal vertical={false} />
          ...
          <div className="mt-4 border-l-4 border-primary bg-background px-4 py-3 rounded-r-lg">
            ...
            <p className="text-xs text-text-secondary italic">
              {spendInsight || 'Create RFQs and approve quotations to see procurement insights.'}
            </p>
          </div>
        </div>
      </div>
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════ */
function ManagerDashboard({ pos, approvals, rfqs, loading, monthlySpend, budget, userName }) {
  const navigate = useNavigate()
        ...
        <div className="col-span-3 bg-surface border border-border rounded-lg p-5">
          ...
          <div className="mt-4 border-t border-border pt-4 flex flex-col gap-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs font-bold text-text-primary">{formatINR(budget?.spent || 0)} / {formatINR(budget?.budgetCap || 0)}</span>
            </div>
            <div className="w-full bg-background rounded-full h-2">
              <div className="bg-warning h-2 rounded-full transition-all duration-500" style={{ width: `${budget?.utilisationPct || 0}%` }} />
            </div>
            <div className="flex justify-between text-xs text-text-muted">
              <span>{budget?.utilisationPct || 0}% utilised</span>
              <span className="font-medium text-text-secondary">{formatINR(budget?.remaining || 0)} remaining</span>
            </div>
          </div>
        </div>
      </div>
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════ */
function VendorDashboard({ pos, quotations, loading, vendorStats }) {
  const navigate = useNavigate()
  const { user } = useAuth()
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-2 bg-surface border border-border rounded-lg p-5">
          ...
          <div className="flex flex-col divide-y divide-border">
            {[
              { icon: Star, label: 'Vendor Rating', value: `${vendorStats?.rating || 0} / 5.0`, color: 'text-warning' },
              { icon: CheckSquare, label: 'On-Time Delivery', value: `${vendorStats?.onTimeDeliveryPct || 0}%`, color: 'text-success' },
              { icon: TrendingUp, label: 'Quote Win Rate', value: `${vendorStats?.winRate || 0}%`, color: 'text-primary' },
              { icon: Clock, label: 'Avg. Response Time', value: `${vendorStats?.avgResponseDays || 0} days`, color: 'text-text-muted' },
          ...
          <div className="mt-4 border-t border-border pt-4">
            ...
            <div className="flex flex-col gap-2">
              {(vendorStats?.openRfqs || []).length === 0 ? (
                <p className="text-xs text-text-muted">No open RFQ invitations right now.</p>
              ) : (
                vendorStats.openRfqs.map((rfq) => (
                  <div key={rfq.id}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-text-muted mt-0.5">Deadline: {formatDate(rfq.deadline)}</p>
                    </div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const a = {}
        if (role === 'admin') {
          const [cat, summary] = await Promise.all([getSpendingByCategory(), getSpendSummary()])
          a.categoryData = cat
          a.spendSummary = summary
        }
        if (role === 'procurement_officer') {
          const [monthly, pipeline, insight] = await Promise.all([
            getMonthlySpend(6), getPipelineSummary(), getSpendInsight(),
          ])
          a.monthlySpend = monthly
          a.pipeline = pipeline
          a.spendInsight = insight.message
        }
        if (role === 'manager') {
          const [monthly, budget] = await Promise.all([getMonthlySpend(6), getBudgetUtilisation()])
          a.monthlySpend = monthly
          a.budget = budget
        }
        if (role === 'vendor') {
          a.vendorStats = await getVendorDashboardStats()
        }
        setAnalytics(a)
      } finally {
        setLoading(false)
  ...
  const props = {
    pos, approvals, rfqs, vendors, quotations, loading,
    categoryData: analytics.categoryData || [],
    spendSummary: analytics.spendSummary,
    activities,
    monthlySpend: analytics.monthlySpend || [],
    pipeline: analytics.pipeline,
    spendInsight: analytics.spendInsight,
