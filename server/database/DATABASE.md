...
Detailed reference for the VendorBridge schema ΓÇö tables, columns, constraints, relationships, triggers, and common queries.
...
| # | Issue | Fix |
|---|-------|-----|
| 1 | `vendors.gstin` had no uniqueness constraint | Added `UNIQUE` ΓÇö prevents two vendors with same GSTIN |
...
| 6 | Missing `ai_summaries` table | Added ΓÇö caches AI comparison results, avoids repeat API calls |
| 7 | Missing `notifications` table | Added ΓÇö supports PS Screen 9 (RFQ, approval, invoice alerts) |
| 8 | Missing `password_reset_tokens` table | Added ΓÇö supports Forgot Password from Login screen |
| 9 | `rfqs` missing `priority` column | Added `priority VARCHAR(10) CHECK (low/medium/high)` |
| 10 | `rfqs` missing `budget` column | Added `budget DECIMAL(12,2)` ΓÇö enables overspend warning |
| 11 | `vendors` missing `contact_person` and `email` | Added both columns |
| 12 | `vendors` missing `state` / `state_code` | Added ΓÇö for CGST vs IGST determination |
| 13 | `quotations` missing `selected_at` / `selected_by` | Added ΓÇö approval timeline display |
| 14 | `purchase_orders` missing `approved_by` | Added FK ΓåÆ users ΓÇö audit trail for PO footer |
...
| 18 | `approvals.level` too restrictive (`L1`/`L2` CHECK) | Changed to `INTEGER DEFAULT 1` ΓÇö supports multi-level workflows |
...
| 21 | `vendors.user_id` ON DELETE CASCADE was too aggressive | Changed to `ON DELETE SET NULL DEFAULT NULL` ΓÇö supports deferred user linking |
| 22 | No vendor invite flow | Added `vendor_invite_token`, `invite_sent_at`, `invite_accepted_at` |
| 23 | `purchase_orders` missing `igst` column | Added ΓÇö required when vendor and org are in different states |
...
rfq_items (1) ----< (N) quotation_items    [rfq_item_id ΓÇö item mapping]
...
|--------|------|-------------|-------------|
| id | SERIAL | PRIMARY KEY | |
| user_id | INTEGER | FK ΓåÆ users, ON DELETE SET NULL, DEFAULT NULL | Linked after invite accepted |
| company_name | VARCHAR(255) | NOT NULL | |
| gstin | VARCHAR(15) | **UNIQUE** | GSTIN ΓÇö uniqueness prevents fake/duplicate vendors |
...
| budget | DECIMAL(12,2) | | Budget cap ΓÇö used for overspend warning on comparison screen |
| created_by | INTEGER | FK ΓåÆ users, ON DELETE SET NULL | |
...
|--------|------|-------------|-------------|
| id | SERIAL | PRIMARY KEY | |
| rfq_id | INTEGER | FK ΓåÆ rfqs, CASCADE | |
...
| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| rfq_id | INTEGER | FK ΓåÆ rfqs, CASCADE, PK | |
| vendor_id | INTEGER | FK ΓåÆ vendors, CASCADE, PK | |
...
| id | SERIAL | PRIMARY KEY | |
| rfq_id | INTEGER | FK ΓåÆ rfqs, CASCADE | |
| vendor_id | INTEGER | FK ΓåÆ vendors, CASCADE | |
...
| selected_by | INTEGER | FK ΓåÆ users, ON DELETE SET NULL | Who selected it |
| created_at | TIMESTAMP | DEFAULT NOW | |
| updated_at | TIMESTAMP | DEFAULT NOW | |
...
|--------|------|-------------|-------------|
| id | SERIAL | PRIMARY KEY | |
| quotation_id | INTEGER | FK ΓåÆ quotations, CASCADE | |
| rfq_item_id | INTEGER | FK ΓåÆ rfq_items, ON DELETE SET NULL | Maps vendor item ΓåÆ original RFQ item |
| item_name | VARCHAR(255) | NOT NULL | |
...
| id | SERIAL | PRIMARY KEY | |
| quotation_id | INTEGER | FK ΓåÆ quotations, CASCADE | |
| rfq_id | INTEGER | FK ΓåÆ rfqs, CASCADE | Denormalized for direct RFQ lookup without extra join |
| approver_id | INTEGER | FK ΓåÆ users, ON DELETE SET NULL | |
| level | INTEGER | NOT NULL DEFAULT 1 | 1, 2, 3ΓÇª supports multi-level workflow |
| status | VARCHAR(20) | CHECK (pending/approved/rejected) | |
| remarks | TEXT | | |
| actioned_at | TIMESTAMP | | When action was taken |
| created_at | TIMESTAMP | DEFAULT NOW | |

> `rfq_id` is auto-populated by `trigger_set_approval_rfq_id` ΓÇö no manual input needed.
...
| quotation_id | INTEGER | FK ΓåÆ quotations, ON DELETE SET NULL, **UNIQUE** | One PO per approved quotation |
| vendor_id | INTEGER | FK ΓåÆ vendors, ON DELETE SET NULL | |
...
| igst | DECIMAL(12,2) | NOT NULL DEFAULT 0 | Used when vendor state Γëá org state |
| grand_total | DECIMAL(12,2) | NOT NULL DEFAULT 0 | |
| status | VARCHAR(30) | CHECK (pending_payment/paid/overdue) | |
| po_date | TIMESTAMP | DEFAULT NOW | |
| due_date | TIMESTAMP | | PO payment due date |
| approved_by | INTEGER | FK ΓåÆ users, ON DELETE SET NULL | Who gave final approval ΓÇö for audit trail |
| created_at | TIMESTAMP | DEFAULT NOW | |
...
|--------|------|-------------|-------------|
| id | SERIAL | PRIMARY KEY | |
| po_id | INTEGER | FK ΓåÆ purchase_orders, CASCADE, **UNIQUE** | One invoice per PO |
...
Immutable audit log ΓÇö no UPDATE or DELETE allowed.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| id | SERIAL | PRIMARY KEY | |
| user_id | INTEGER | FK ΓåÆ users, ON DELETE SET NULL | |
...
| id | SERIAL | PRIMARY KEY | |
| vendor_id | INTEGER | FK ΓåÆ vendors, CASCADE | |
| po_id | INTEGER | FK ΓåÆ purchase_orders, CASCADE | |
| rating | INTEGER | NOT NULL CHECK (1-5) | Raw input (1, 2, 3, 4, 5) |
| review | TEXT | | |
| rated_by | INTEGER | FK ΓåÆ users, ON DELETE SET NULL | |
| created_at | TIMESTAMP | DEFAULT NOW | |
...
|--------|------|-------------|-------------|
| id | SERIAL | PRIMARY KEY | |
| rfq_id | INTEGER | FK ΓåÆ rfqs, CASCADE | The RFQ being summarized |
| summary | TEXT | NOT NULL | AI-generated summary output |
| recommended_vendor_id | INTEGER | FK ΓåÆ vendors, ON DELETE SET NULL | Vendor the AI recommends |
| reasoning | TEXT | | Explanation for recommendation |
| generated_at | TIMESTAMP | DEFAULT NOW | |
| generated_by | INTEGER | FK ΓåÆ users, ON DELETE SET NULL | User who triggered the summary |

---

### notifications

Drives PS Screen 9 ΓÇö RFQ notifications, approval alerts, invoice updates.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| id | SERIAL | PRIMARY KEY | |
| user_id | INTEGER | FK ΓåÆ users, CASCADE | Recipient |
...
|--------|------|-------------|-------------|
| id | SERIAL | PRIMARY KEY | |
| user_id | INTEGER | FK ΓåÆ users, CASCADE | |
...
```
vendor_ratings.rating  ΓåÆ INTEGER CHECK (1-5)     ΓÇö raw user input (star selection)
vendors.rating         ΓåÆ DECIMAL(3,2) DEFAULT 0   ΓÇö computed average, e.g. 4.50
```
...
- If `vendor.state_code = org_profile.org_gstin[0:2]` ΓåÆ intra-state ΓåÆ split into CGST + SGST
- If states differ ΓåÆ inter-state ΓåÆ use IGST only
...
2. **SQL Injection:** Always use parameterized queries ΓÇö never string interpolation
3. **RBAC:** Role checks enforced at API middleware layer
4. **Audit Trail:** `activity_logs` is append-only ΓÇö block UPDATE/DELETE via application policy
...
4. `ai_summaries` avoids repeat API calls ΓÇö always check for existing summary before generating
...
| `idx_vendors_user_id` | vendors | user_id | Join vendors ΓåÆ users |
...
| `idx_quotation_items_rfq_item_id` | quotation_items | rfq_item_id | Vendor item ΓåÆ RFQ item mapping |
